...
\begin{figure}[htbp]
	...
	\caption{Installation von Erlang im Dockerfile}\cite{dock:1}
...
\begin{figure}[htbp]
	...
	\caption{Konfiguration der Variablen bei Heroku}\cite{hero:1}
	\label{img:herokuConf}
\end{figure} 


\subsection{Jenkins}\label{Jenkins}
Jenkins ist ein webbasiertes Continuous Integration Tool. Es wurde von Sun in Java entwickelt und wird von Oracle bis heute weiterentwickelt. In diesem Projekt wird Jenkins verwendet, um die CEP und die WetterAPI-Applikation automatisch zu erzeugen und auf Cloud Foundry zu deployen und zu starten.
Dies geschieht mit Hilfe des Cloud Foundry Plugins für Jenkins. Dazu wird der aktuelle Stand des Git-Repository auf einem Server mit Hilfe des Terminalbefehls mvn install gebaut. Wurde das Projekt erfolgreich gebaut, wird das War-File (WetterAPI) oder das Jar-File (CEP) mit den für die einzelnen Applikationen spezifischen Einstellungen und Umgebungsvariablen (siehe Abb. \ref{img:jenkinsConf} ) auf Cloud Foundry gepusht und gestartet.
\begin{figure}[htbp]
	\centering
